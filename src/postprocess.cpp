...
void nms(std::vector<Detection>& res, float* output, float conf_thresh, float nms_thresh) {
    ...
    // The TensorRT YOLO plugin increments output[0] for every candidate before
    // checking its fixed output capacity.  Consequently output[0] can be larger
    // than the number of Detection records that were actually written.  Never
    // trust that counter as an array bound: doing so reads beyond the pinned host
    // buffer on busy frames and eventually terminates Windows with 0xC0000409.
    const float raw_count = output[0];
    if (!std::isfinite(raw_count) || raw_count <= 0.0f) {
        return;
    }
    const int count = static_cast<int>((std::min)(
        raw_count, static_cast<float>(kMaxNumOutputBbox)));
